...
router.post("/inventories", async (req, res) => {
    try {
        ...
        const { error, value: cleanedData } =
            helpers.inventorySchema.validate(itemBody);
        if (error) {
            ...
            console.log(
                "Successfully added an inventory item with id ",
                newInventoryItemId
            );
            const newInventoryItemData = await knex('inventories').select('*').where({id: newInventoryItemId[0]});
            res.status(201).json(newInventoryItemData);
        }
    } catch (error) {
...
router.put("/inventories/:id", async (req, res) => {
    ...
    try {
        const currentItemData = await knex("inventories")
            .select("*")
            .where({ id: itemId });
        ...
        if (error) {
            ...
            const updatedItemData = await knex("inventories")
                .select("*")
                .where({ id: itemId });
...
router.delete("/inventories/:id", async (req, res) => {
    ...
    try {
        const deletedItemData = await knex("inventories")
            .select("*")
            .where({ id: itemId });
